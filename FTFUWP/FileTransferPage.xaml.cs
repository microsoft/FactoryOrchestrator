...
namespace Microsoft.FactoryTestFramework.UWP
{
    ...
    public sealed partial class FileTransferPage : Page
    {
        ...
        private async void ConfirmCopy_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (sending)
            {
                await FactoryTestFramework.UWP.FileTransferHelper.SendFileToServer(ClientFileTextBox.Text, ServerFileTextBox.Text);
            }
            else
            {
                await FactoryTestFramework.UWP.FileTransferHelper.GetFileFromServer(ServerFileTextBox.Text, ClientFileTextBox.Text);
            }
